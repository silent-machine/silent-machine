...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Silent-Machine thinks for you about the meanings.",
        epilog="Example:  silent-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | silent-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | silent-machine         #

    $ silent-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="silent-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"silent-machine: {utterance}.")
        syslog.closelog()
